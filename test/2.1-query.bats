...
@test "If called with a service name, spoon should query aws filtering for the service_name tag." {
    run $spoon foo
    assert_equal_regex "$(mock_get_call_args $mock_aws_path 1)" '--filters Name=tag:Name,Values=*foo*'
}

@test "If called with an instance-id, spoon should query aws filtering for instance id." {
    run $spoon i-foo
    assert_equal_regex "$(mock_get_call_args $mock_aws_path 1)" '--instance-ids i-foo'
}
