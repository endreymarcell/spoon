...
@test "If trying to filter for prod and preprod at the same time, spoon should complain and exit with 1." {
    run $spoon -p -P foo
    assert_failure
    assert_output "[spoon] Invalid arguments: -P/--prod and -p/--preprod are mutually exclusive."
}

@test "If trying to SSH to the first and all instances at the same time, spoon should complain and exit with 1." {
    run $spoon -1 -a foo
    assert_failure
    assert_output "[spoon] Invalid arguments: -1/--first and -a/--all are mutually exclusive."
}
